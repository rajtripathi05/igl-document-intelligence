"""Per-document Excel export for the Sales Order (Purchase Order) processor.

Migrated verbatim from the V1.2 ``excel.ExcelGenerator`` — the two-sheet
workbook ("Purchase Order Summary" + "Line Items") is byte-for-byte equivalent
to the prior export. Excel is always generated from standardized JSON, never
directly from Gemini output (see CLAUDE.md). The module exposes
``build_excel_bytes(data)`` for the generic engine.
"""
...
class ExcelGenerator:
    """Generate two-sheet Purchase/Sales Order Excel workbooks from JSON."""

    def build_workbook(self, purchase_order: dict[str, Any]) -> Workbook:
        """Build a workbook from standardized order JSON."""
        workbook = Workbook()
    ...
    def generate_purchase_order_excel(
        ...
        """Generate and save an order Excel file."""
    ...
    def _write_pairs(self, sheet: Worksheet, row: int, pairs: list[tuple[str, Any]]) -> int:
...
def build_excel_bytes(data: dict[str, Any]) -> bytes:
    """Module entry point used by the generic engine."""
