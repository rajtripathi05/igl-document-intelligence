"""Per-document Excel export for the Shipping Bill processor.

Migrated verbatim from the V1.2 ``excel_shipping_bill.ShippingBillExcelGenerator``
— the two-sheet workbook ("Shipping Bill Summary" + "Line Items") is byte-for-
byte equivalent to the prior export. Excel is always generated from standardized
JSON, never directly from Gemini output. Exposes ``build_excel_bytes(data)``.
"""
    ...
    def _build_line_items_sheet(self, sheet: Worksheet, data: dict[str, Any]) -> None:
        ...
        for row_index, item in enumerate(items, start=2):
            ...
            for col_index, (key, _) in enumerate(_LINE_ITEM_COLUMNS, start=1):
                ...
                cell.border = _BORDER
                cell.alignment = Alignment(vertical="top", wrap_text=key == "product_description")
