"""Validation engine for standardized Shipping Bill data.

Completely independent from Purchase Order validation. The schema in
``schemas/shipping_bill_schema.json`` is the single source of truth for the
normalized structure. Validation is performed in Python, never by Gemini.
"""
...
def validate_shipping_bill(data: dict[str, Any]) -> list[str]:
